 ...
 * ========================================================= */
 ...
 (function($) {
	...
	Calendar.prototype = {
							...
							$(this).find('.day-content').each(function() {
								...
								if((_this.options.minDate == null || currentDate >= _this.options.minDate) && (_this.options.maxDate == null || currentDate <= _this.options.maxDate))
								{
									...
									if(dayData.length > 0)
									{
										...
										//添加链接
										_this._addURL($(this),_this._getURL($(this)));
		...
		_addURL: function(adoptor,URL){
		 	adoptor.attr("url",URL).click(function(){window.location.href= $(this).attr('url');});
		},
		_getURL: function(adoptor){
			var year = $('.year-title:not(.hidden-xs)').text();//year
			var month = adoptor.parent().parent().parent().siblings().children().find(".month-title").text();//month
			var day = adoptor.text();
			day = day.length==2 ? day : '0'+day;
			return "today?"+year+'-'+getColorMonth(month)+'-'+day;
		},
		_renderDataSourceDay: function(elt, events) {
			switch(this.options.style)
			{
				...
				case 'background':
					...
					break;
				case 'url':
					elt.parent().attr('url',events[events]);
					break;
			}
		},
		_applyEvents: function () {
			...
			this.element.find('.calendar-header .prev').click(function() {
				if(!$(this).hasClass('disabled')) {
					_this.element.find('.months-container').animate({'margin-left':'100%'},100, function() {
						...
						getEvent($(this).parent().find('.year-title:not(.hidden-xs)').prev().text());
					});
				}
			});
			...
			this.element.find('.calendar-header .next').click(function() {
				if(!$(this).hasClass('disabled')) {
					...
					getEvent($(this).parent().find('.year-title:not(.hidden-xs)').next().text());
				}
			});
	...
	//请求函数
		function getEvent(startyears){
			json = {dataSource:[{}]};
				$.ajax({
				   	url:"../Front/getYearActivity",
				   	data: {'startdate':startyears},
				   	success: function(event){	   			
				   			$.each(event.data,function(i,item){
				   				startyear = item.activitydate.substring(0,4);
				   				if(item.activitydate.substring(5,6) == 0){
				   					startmonth = parseInt(item.activitydate.substring(6,7)) - 1;
				   				}else{
				   					startmonth = parseInt(item.activitydate.substring(5,7)) - 1;
				   				}
				   				startday = item.activitydate.substring(8,10);
				   				endyear = item.activitydate.substring(0,4);
				   				if(item.activitydate.substring(5,6) == 0){
				   					endmonth = parseInt(item.activitydate.substring(6,7)) - 1;
				   				}else{
				   					endmonth = parseInt(item.activitydate.substring(5,7)) - 1;
				   				}
				   				endday = item.activitydate.substring(8,10);
				   				json.dataSource[i]={startDate: new Date(startyear,startmonth,startday),
		...
		//初始化
		getEvent(new Date().getFullYear());
		function getColorMonth(args){
			switch (args){
				case '一月':
				args = '01';
				break;
